...
class Complie {
    ...
    compileText(node) {
        ...
        //RegExp.$1是匹配分组部分
        const exp = RegExp.$1
        console.log(this.$vm[RegExp.$1], 'text');

        //    node.textContent = this.$vm[RegExp.$1]
        this.update(node, this.$vm, exp, 'text')
    }
    compileElement(node) {
        // 查看node的特性中，是否有x-xx， @xx
    ...
    //编写update函数，它可复用
    //exp是表达式，dir是具体操: text, html.model
